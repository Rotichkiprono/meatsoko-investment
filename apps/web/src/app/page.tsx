      ...
      if (provider && idToken) {
        try {
          ...
          if (apiUrl) {
            ...
            setSyncStatus('Wallet connected. Backend sync is not configured yet.');
          }
        } catch (error) {
  ...
  return (
    <main className="min-h-screen flex flex-col items-center justify-center p-6 bg-slate-50 font-sans">
      <div className="max-w-md w-full bg-white rounded-2xl shadow-xl p-8 text-center border border-slate-100">
        ...
        <p className="text-slate-500 mb-8">Tokenized Agricultural Assets on Hedera</p>

        {!provider ? (
          <button onClick={login} className="w-full bg-blue-600 text-white font-semibold py-3 px-4 rounded-xl hover:bg-blue-700 transition shadow-sm">
            Sign In with Google
          </button>
        ) : (
          <div className="space-y-6">
            ...
            <div className="text-left">
              <label className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Your EVM Address</label>
              <div className="mt-1 bg-slate-100 p-3 rounded-lg text-sm text-slate-700 font-mono break-all">{address || 'Deriving address...'}</div>
            </div>

            <div className="text-left">
              <label className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Status</label>
              <div className="mt-1 text-sm font-medium text-slate-600">{syncStatus}</div>
            </div>
            ...
            <div className="pt-4 space-y-3">
              ...
              <button onClick={logout} className="w-full bg-white border border-slate-200 text-slate-700 font-semibold py-3 px-4 rounded-xl hover:bg-slate-50 transition">
                Disconnect
              </button>
            </div>
          </div>
